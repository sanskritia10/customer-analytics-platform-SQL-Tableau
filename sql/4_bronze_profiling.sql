/*========================================================
  DATA PROFILING
  Layer: Bronze
  Purpose: Understand structure, completeness,
           uniqueness, distributions and ranges
========================================================*/
...
SELECT customer_state, customer_city, COUNT(*) AS customers
FROM bronze.olist_customers
GROUP BY customer_state, customer_city
ORDER BY customers DESC;

select count(distinct customer_city) from bronze.olist_customers;
...
SELECT
    ...
    ROUND(
        PERCENTILE_CONT(0.50)
        WITHIN GROUP (ORDER BY freight_value)::numeric,
        2
    ) AS median_freight
FROM bronze.olist_order_items;
...
SELECT
    ...
    ROUND(
        PERCENTILE_CONT(0.50)
        WITHIN GROUP (ORDER BY product_weight_g)::numeric,
        2
    ) AS median_weight
FROM bronze.olist_products;
...
SELECT
	...
	round(100 * COUNT(review_comment_title)/COUNT(*),2) AS perc_reviews_with_title,
    COUNT(review_comment_message) AS reviews_with_message,
	round(100 * COUNT(review_comment_message)/count(*),2) AS perc_reviews_with_message,
    COUNT(*) - COUNT(review_comment_message) AS reviews_without_message,
	round(100 * (COUNT(*) - COUNT(review_comment_message))/count(*),2) AS perc_reviews_without_message
FROM bronze.olist_order_reviews;
...
SELECT
    ...
    ) AS missing_geolocation_zip_code_prefix,
    COUNT(*) FILTER (
        WHERE geolocation_lat IS NULL
    ) AS missing_geolocation_lat,
    COUNT(*) FILTER (
        WHERE geolocation_lng IS NULL
    ) AS missing_geolocation_lng,
    COUNT(*) FILTER (
        WHERE geolocation_city IS NULL
    ) AS missing_geolocation_city,
    COUNT(*) FILTER (
        WHERE geolocation_state IS NULL
    ) AS missing_geolocation_state
FROM bronze.olist_geolocation;
...
SELECT
    ...
    ) AS missing_review_comment_title,
    COUNT(*) FILTER (
        WHERE review_comment_message IS NULL
    ) AS missing_review_comment_message,
    COUNT(*) FILTER (
        WHERE review_creation_date IS NULL
    ) AS missing_review_creation_date,
    COUNT(*) FILTER (
        WHERE review_answer_timestamp IS NULL
    ) AS missing_review_answer_timestamp
FROM bronze.olist_order_reviews;
...
SELECT
    ...
    ) AS missing_seller_zip_code_prefix,
    COUNT(*) FILTER (
...
SELECT
    ...
    ) AS missing_product_category_name_english
FROM bronze.product_category_translation;
